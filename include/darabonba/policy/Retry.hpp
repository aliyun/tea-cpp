...
//      DARABONBA_PTR_FROM_JSON(backoff, backoff_);
  ...
  public:
    ...
    friend void from_json(const Darabonba::Json &j, RetryPolicyContext &obj) {
      DARABONBA_FROM_JSON(retriesAttempted, retriesAttempted_);
      DARABONBA_FROM_JSON(exception, exception_);
    ...
    // 构造函数初始化
    RetryPolicyContext(int retriesAttempted, Exception httpException)
    ...
    RetryPolicyContext& setException(Exception ex) { exception_ = ex; return *this; }
