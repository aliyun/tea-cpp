...
TEST_F(ExceptionTest, InitValidateExceptionWithCodeAndMessage) {
    // ValidateException 构造函数参数是 (code, message)，name_ 固定为 "ValidateException"
    ...
    EXPECT_NE(ex.getMessage().find("Field 'name' is required"), std::string::npos);
}
...
TEST_F(ExceptionTest, ValidateExceptionInheritance) {
    ...
    EXPECT_NE(baseRef.getMessage().find("Required field missing"), std::string::npos);
}
...
TEST_F(ExceptionTest, ThrowAndCatchRetryError) {
    try {
        ...
        EXPECT_NE(std::string(e.what()).find("Max retries exceeded"), std::string::npos);
    }
}
